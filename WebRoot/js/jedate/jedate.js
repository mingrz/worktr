...
(function(win) {
	...
	*/
    ...
    jeDt.val = function(elem, value) {
        if (value !== undefined && elem.nodeType === 1) {
            elem.value = value;
            if($('#upTimeIpt')&&$('#downTimeIpt')){
                var upTimeIpt = $('#upTimeIpt');
                var downTimeIpt = $('#downTimeIpt');
                var divTitleText=$(".divTitle span").text();
                if(upTimeIpt.val()!=upTimeIpt.attr('data')||downTimeIpt.val()!=downTimeIpt.attr('data')){
                    if(divTitleText=="分时数据指数"){
                        DataTimeAjax();
                        upTimeIpt.attr('data',upTimeIpt.val());
                    }else{
                        TCR_dataAjax();
                        upTimeIpt.attr('data',upTimeIpt.val());
                        downTimeIpt.attr('data',downTimeIpt.val());
                    } 
                }
            }
    ...
    InitDate.prototype = {
        ...
        orien:function(obj, self, pos) {
            ...
            tops =  (rect.bottom + obj.offsetHeight  <= jeDt.winarea()) ?
                rect.bottom - 1 : rect.top > obj.offsetHeight  ? rect.top - obj.offsetHeight + 1 :jeDt.winarea() - obj.offsetHeight;
                ...
                jeDt.on(cls, "click", function(ev) {
                    ...
                    if (opts.choosefun === "function" || opts.choosefun != null) {
                     ...
                     if($("#DD")){
                        ...
                        var divMap=$("#map");
                        if(_dd!=$("#DD").attr("data")){
                           if(divMap.attr("roadFlag")=="拥堵水平"||divMap.attr("roadFlag")=="日拥堵率"||divMap.attr("roadFlag")=="拥堵比列"||divMap.attr("roadFlag")=="拥堵时间"||divMap.attr("roadFlag")=="日稳定性"){
                                initTimeline(1,1,0);
                            }else if(divMap.attr("roadFlag")=="运行状态"||divMap.attr("roadFlag")=="拥堵路段"||divMap.attr("roadFlag")=="服务水平"||divMap.attr("roadFlag")=="流量"||divMap.attr("roadFlag")=="拥堵瓶颈"||divMap.attr("roadFlag")=="排队长度"||divMap.attr("roadFlag")=="动态安全"||divMap.attr("roadFlag")=="天气安全"||divMap.attr("roadFlag")=="道路尾气"){
                                initTimeline(1,1,1);  
                                ...
                                initTimeline(1,1,70);  
                            }else if(divMap.attr("roadFlag")=="单点"||divMap.attr("roadFlag")=="协调"||divMap.attr("roadFlag")=="网格排放"){
                                initTimeline(1,1); 
